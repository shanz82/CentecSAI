    ...
    for (; i < sai_metadata_all_enums_count; ++i)
    {
        ...
        for (; j < emd->valuescount; ++j)
        {
            ...
            if (value != last + 1)
            {
                ...
                {
                    META_ENUM_ASSERT_FAIL(emd, "values are not increasing by 1: last: %d, name: %s, current: %d,name: %s should be marked as @flags?", last,emd->valuesnames[j-1], value,  emd->valuesnames[j]);
                }
            }
